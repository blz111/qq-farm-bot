...
- **自动种植** — 收获/铲除后自动购买最优经验种子并种植（自动计算有肥/无肥最优，普通肥料 <10 时切换无肥方案）
...
- **最优种子提示** — 状态栏一行展示当前等级/地块数的最优种子（无肥/有肥）
...
```bash
# 经验收益率分析（推荐，基于 seed-shop 数据）
node tools/calc-exp-yield.js --lands 18 --level 24
